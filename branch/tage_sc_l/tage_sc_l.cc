...
#define NHIST 8 // twice the number of different histories
...
bool tage_sc_l::predict_branch(champsim::address ip)
{
  ...
  // }
  return was_predicted_taken = predictor.GetPrediction(ip.to<unsigned long>());
}
...
void tage_sc_l::last_branch_result(champsim::address ip, champsim::address branch_target, bool taken, uint8_t branch_type)
{
  if (branch_type == BRANCH_CONDITIONAL) {
    predictor.UpdatePredictor(ip.to<unsigned long>(), branch_type, taken, was_predicted_taken, branch_target.to<unsigned long>());
  } else {
    predictor.TrackOtherInst(ip.to<unsigned long>(), branch_type, taken, branch_target.to<unsigned long>());
  }
}
